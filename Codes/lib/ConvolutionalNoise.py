...
class _Conv_Noise(Layer):
    ...
    def __init__(self, rank,
                 ...
                 **kwargs):
        super(_Conv_Noise, self).__init__(**kwargs)
        self.rank = rank
    ...
    def call(self, inputs, training=None):
        ...
        if self.rank == 3:
            outputs = tf.keras.backend.conv3d(
                inputs,
                self.kernel,
                strides=self.strides,
                padding=self.padding,
        ...
        def noised_outputs():
                if self.sigmanoise >0:
                        noise = tf.random.normal(shape=tf.shape(outputs), mean=0.,stddev=self.sigmanoise)
                        # now same line using tf2
                        return outputs + noise #K.bias_add(output, noise, data_format='channels_last')
                return outputs
        
        outputs = tf.keras.backend.in_train_phase(outputs, noised_outputs, training=training)
    ...
    def get_config(self):
        ...
        base_config = super(_Conv_Noise, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


class Conv2DNoise(_Conv_Noise):
    ...
    def get_config(self):
        config = super(Conv2DNoise, self).get_config()
        config.pop('rank')
        return config
