...
# https://github.com/Joool/cleverhans/blob/cc239194feaa2ed62296d58b100b5d1ee2651ddc/cleverhans/tf2/attacks/carlini_wagner_l2.py
# https://github.com/Joool/cleverhans/blob/cc239194feaa2ed62296d58b100b5d1ee2651ddc/cleverhans/tf2/utils.py
...
cte_init = args.cte_init        # CW2 constant initialization
cte_update = args.cte_update    # CW2 constant update
kappa = args.kappa              # CW2 kappa constant

np_l2=np.zeros(shape=(n_succ))

# Define the attack
cw2_attack = CarliniWagnerL2( model,
                                initial_const=cte_init,
                                learning_rate=cte_update,
...
if not os.path.isfile(path_attack): # Generate and save adversarial examples corresponding to X_test
    ...
    while i_succ != n_succ:
        ...
        img = np.reshape(img0, (1, img_rows, img_cols, img_channels))

        i_nat = np.argmax(model.predict(img))
        if np.argmax(Y_test[idx]) != i_nat: # Wrong classification
...
if (not (os.path.isfile(path_attack) and overwrite == False)) and save:
    np.savez(path_attack, dist_adv_ex_mat = dist_adv_ex_mat, rho = rho, p_succ = p_succ, i_succ = i_succ, n_classified = n_classified, max_iter = max_iter, cte_init = cte_init, cte_update = cte_update, kappa = kappa, X_test_nat_adv = X_test_nat_adv, X_test_nat_adv_pred = X_test_nat_adv_pred)
